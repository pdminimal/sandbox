...
orig = re.sub(r'(［.+?］|《.+?》)', '', jaconv.h2z(text, digit=True, ascii=True),
                   flags=(re.MULTILINE | re.DOTALL))
import random
def make_next_segs(orig, one_grams, thres=1000):
    one_gram_counts = Counter(one_grams).most_common(thres)

    two_grams = [''.join(one_grams[i: i + 2]) for i in range(0, len(one_grams), 2)]
    two_grams2 = [''.join(one_grams[i: i + 2]) for i in range(1, len(one_grams), 2)]
    ...
    segs = [i for i in segs[:thres] if i[1] > int(len(one_grams) / thres / 5 + random.random())]
...
print(orig[2000:2300])
print([jaconv.z2h(i, digit=True, ascii=True) for i in segs[2000:2300]])
